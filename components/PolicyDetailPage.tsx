...
const PolicyDetailPage: React.FC<PolicyDetailPageProps> = ({
  ...
  const buildPolicyClause = (
    record: Clause | InsuranceProduct,
    clauseType: "主险" | "附加险",
  ): PolicyClause => {
    const coverageDetails: CoverageItem[] = record.coverageDetails || [];
    const totalSumInsured = coverageDetails.reduce(
      (sum, item) => sum + parseSumInsured(item.amount),
      0,
    );
    const annualPremium =
      "annualPremium" in record ? (record as any).annualPremium : 0;
    const deductible =
      "deductible" in record ? (record as any).deductible : undefined;
  ...
  const handleProductSelect = (selectedProductCode: string) => {
    ...
    if (product) {
      ...
      const clauseRecord = clauses.find(
        (c) => c.productCode === product.productCode,
      );
  ...
  const [specialAgreements, setSpecialAgreements] = useState<
    SpecialAgreement[]
  >(policy?.specialAgreements || []);

  // 免赔约定
  const [deductionRules, setDeductionRules] = useState<DeductionRule[]>(
    policy?.deductionRules || [],
  );

  // 保单明细表
  const [schedule, setSchedule] = useState<PolicySchedule | undefined>(
    policy?.schedule,
  );
  ...
  const [editingAgreement, setEditingAgreement] =
    useState<SpecialAgreement | null>(null);
  const [editingDeduction, setEditingDeduction] =
    useState<DeductionRule | null>(null);
  ...
  const handleSaveAgreement = () => {
    if (editingAgreement) {
      setSpecialAgreements(
        specialAgreements.map((a) =>
          a.id === tempAgreement.id ? tempAgreement : a,
        ),
      );
    } else {
    ...
    if (editingDeduction) {
      setDeductionRules(
        deductionRules.map((d) =>
          d.id === tempDeduction.id ? tempDeduction : d,
        ),
      );
    } else {
  ...
  const handleGenerateSchedule = async () => {
    try {
      const result = await api.generatePolicySchedule(
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <Select
            ...
            options={Object.values(PolicyStatus).map((s) => ({
              label: s,
              value: s,
            }))}
            disabled={!isEditing}
          ...
          <Select
            ...
            options={products.map((p) => ({
              label: `${p.regulatoryName}（${p.companyName}）`,
              value: p.productCode,
            }))}
            placeholder="请选择关联产品"
            disabled={!isEditing}
          ...
          <Input
            ...
            type="date"
            value={issueDate}
            onChange={(e) => isEditing && setIssueDate(e.target.value)}
            disabled={!isEditing}
          />
          <Input
            label="缴费截止日期"
            ...
            onChange={(e) => isEditing && setPaymentDueDate(e.target.value)}
            disabled={!isEditing}
          />
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <Select
            ...
            onChange={(value) =>
              isEditing &&
              setPolicyholder({ ...policyholder, idType: value as any })
            }
            options={[
              { label: "身份证", value: "身份证" },
              { label: "护照", value: "护照" },
          ...
          <Input
            ...
            onChange={(e) =>
              isEditing &&
              setPolicyholder({ ...policyholder, idNumber: e.target.value })
            }
            disabled={!isEditing}
          />
          <Select
            ...
            onChange={(value) =>
              isEditing &&
              setPolicyholder({ ...policyholder, gender: value as any })
            }
            options={[
              { label: "男", value: "男" },
              { label: "女", value: "女" },
          ...
          <Input
            ...
            onChange={(e) =>
              isEditing &&
              setPolicyholder({ ...policyholder, birthDate: e.target.value })
            }
            disabled={!isEditing}
          />
          <Input
          ...
          <Input
            ...
            onChange={(e) =>
              isEditing &&
              setPolicyholder({ ...policyholder, address: e.target.value })
            }
            disabled={!isEditing}
          />
        </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        {insureds.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-sm text-gray-500 mb-4">暂无被保险人信息</p>
            {isEditing && (
              <button
                onClick={() =>
                  setInsureds([
          ...
          <div className="space-y-6">
            {insureds.map((insured, idx) => (
              <div key={idx} className="border border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {isEditing && insureds.length > 1 && (
                    <button
                      onClick={() =>
                        setInsureds(insureds.filter((_, i) => i !== idx))
                      }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  <Input
                    ...
                    onChange={(e) =>
                      ...
                      setInsureds(
                        insureds.map((ins, i) =>
                          i === idx ? { ...ins, name: e.target.value } : ins,
                        ),
                      )
                    }
                    disabled={!isEditing}
                  ...
                  <Select
                    label="证件类型"
                    value={insured.idType}
                    onChange={(value) =>
                      isEditing &&
                      setInsureds(
                        insureds.map((ins, i) =>
                          i === idx ? { ...ins, idType: value as any } : ins,
                        ),
                      )
                    }
                    options={[
                  ...
                  <Select
                    ...
                    onChange={(value) =>
                      ...
                      setInsureds(
                        insureds.map((ins, i) =>
                          i === idx ? { ...ins, gender: value as any } : ins,
                        ),
                      )
                    }
                    options={[
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      ...
                      setInsureds(
                        insureds.map((ins, i) =>
                          i === idx ? { ...ins, phone: e.target.value } : ins,
                        ),
                      )
                    }
                    disabled={!isEditing}
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      ...
                      setInsureds(
                        insureds.map((ins, i) =>
                          i === idx
                            ? { ...ins, relationship: e.target.value }
                            : ins,
                        ),
                      )
                    }
                    disabled={!isEditing}
                    placeholder="本人/配偶/子女/父母"
                  />
                  <Input
                    label="邮箱"
                    value={insured.email || ""}
                    onChange={(e) =>
                      ...
                      setInsureds(
                        insureds.map((ins, i) =>
                          i === idx ? { ...ins, email: e.target.value } : ins,
                        ),
                      )
                    }
                    disabled={!isEditing}
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      ...
                      setInsureds(
                        insureds.map((ins, i) =>
                          i === idx ? { ...ins, address: e.target.value } : ins,
                        ),
                      )
                    }
                    disabled={!isEditing}
            ...
            {isEditing && (
              <button
                ...
                className="w-full py-3 border-2 border-dashed border-gray-300 rounded-lg text-sm text-gray-600 hover:border-brand-blue-500 hover:text-brand-blue-600 transition-colors"
              >
                + 添加更多被保险人
              </button>
            )}
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <Input
            ...
            value={totalPremium}
            onChange={(e) =>
              isEditing && setTotalPremium(parseFloat(e.target.value) || 0)
            }
            disabled={!isEditing}
          />
          <Select
          ...
          <Input
            ...
            value={paidPremium || ""}
            onChange={(e) =>
              isEditing && setPaidPremium(parseFloat(e.target.value) || 0)
            }
            disabled={!isEditing}
          />
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <Input
            label="条款代码"
            value={mainClause.clauseCode}
            onChange={(e) =>
              isEditing &&
              setMainClause({ ...mainClause, clauseCode: e.target.value })
            }
            disabled={!isEditing}
          />
          <Input
            label="条款名称"
            value={mainClause.clauseName}
            onChange={(e) =>
              isEditing &&
              setMainClause({ ...mainClause, clauseName: e.target.value })
            }
            disabled={!isEditing}
          />
          <Input
          ...
          <Input
            label="免赔说明"
            value={mainClause.deductible || ""}
            onChange={(e) =>
              isEditing &&
              setMainClause({ ...mainClause, deductible: e.target.value })
            }
            disabled={!isEditing}
          />
        </div>
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="text-right">
                    <span className="text-sm text-gray-600 mr-2">
                      保额: ¥{clause.sumInsured.toLocaleString()}
                    </span>
                    <span className="text-sm font-medium text-gray-900">
                      保费: ¥{clause.premium.toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {isEditing && (
            <button
              ...
              className="px-3 py-1.5 text-sm bg-brand-blue-600 text-white rounded-md hover:bg-brand-blue-700"
            >
              + 添加约定
            </button>
          )}
                  ...
                  {isEditing && (
                    <div className="space-x-2">
                      <button
                        ...
                        className="text-sm text-brand-blue-600 hover:text-brand-blue-900"
                      >
                        编辑
                      </button>
                      <button
          ...
          <div className="space-y-4">
            {deductionRules.map((rule, idx) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <span className="text-xs px-2 py-0.5 bg-orange-100 text-orange-700 rounded mr-2">
                      {rule.type}
                    </span>
                    <span className="font-semibold text-gray-900">
                      {rule.name}
                  ...
                  {isEditing && (
                    <div className="space-x-2">
                      <button
                        onClick={() => handleEditDeduction(rule)}
                        className="text-sm text-brand-blue-600 hover:text-brand-blue-900"
                      >
                        编辑
                      </button>
                      <button
                        onClick={() => handleDeleteDeduction(rule.id)}
                        className="text-sm text-red-600 hover:text-red-900"
                      >
                        删除
                      </button>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-gray-50 rounded-md">
              ...
              <div>
                ...
                <p className="font-semibold">
                  {schedule.generatedAt.split("T")[0]}
                </p>
              </div>
              <div>
                <span className="text-sm text-gray-500">总保额</span>
                <p className="font-semibold text-blue-600">
                  ¥{schedule.totalSumInsured.toLocaleString()}
                </p>
              </div>
              <div>
                <span className="text-sm text-gray-500">总保费</span>
                <p className="font-semibold text-green-600">
                  ¥{schedule.totalPremium.toLocaleString()}
                </p>
              </div>
            </div>
                ...
                {schedule.items.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-right text-gray-600">
                      ¥{item.deductible.toLocaleString()}
                    </td>
                    <td className="px-4 py-3 text-sm text-right text-gray-600">
                      {item.reimbursementRatio}%
                    </td>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          {isEditing ? (
            <>
              <button
                onClick={() => (policy ? setIsEditing(false) : onBack())}
      ...
      >
        <div className="space-y-4">
          <Input
            ...
            onChange={(e) =>
              setTempAgreement({ ...tempAgreement, title: e.target.value })
            }
            required
          />
          <Select
            ...
            onChange={(value) =>
              setTempAgreement({ ...tempAgreement, category: value as any })
            }
            options={[
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setTempAgreement({ ...tempAgreement, content: e.target.value })
              }
              rows={4}
      ...
      >
        <div className="space-y-4">
          <Input
            ...
            onChange={(e) =>
              setTempDeduction({ ...tempDeduction, name: e.target.value })
            }
            required
          />
          <Select
            ...
            onChange={(value) =>
              setTempDeduction({ ...tempDeduction, type: value as any })
            }
            options={[
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Select
              ...
              onChange={(value) =>
                setTempDeduction({ ...tempDeduction, unit: value as any })
              }
              options={[
                { label: "元", value: "元" },
                { label: "%", value: "%" },

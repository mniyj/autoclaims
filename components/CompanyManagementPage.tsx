  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        ...
        // Map InsuranceCompanyProfile or CompanyListItem to CompanyListItem for display
        const mappedData: CompanyListItem[] = data.map((item: any) => {
          if (item.basicInfo) {
            // It's a full profile
            return {
              code: item.code,
              ...
              registeredCapital: `${item.basicInfo.registeredCapital.value} ${item.basicInfo.registeredCapital.unit}`,
              status: '生效' // Default status
            };
          }
          return item; // It's already a list item (legacy/mock)
        });

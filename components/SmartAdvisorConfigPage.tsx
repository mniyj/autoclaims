...
const SmartAdvisorConfigPage: React.FC = () => {
    ...
    const renderInputGroup = (label: string, value: number, field: string, unit: string) => (
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 min-h-[600px] flex flex-col">
            <div className="px-8 py-6 border-b border-gray-100">
                 ...
                 <p className="text-sm text-gray-500 mt-1">调整各项保险产品的计算参数与默认值</p>
            </div>
            ...
            <div className="p-10 flex-1 overflow-y-auto">
                <div className="space-y-10 max-w-5xl">
                    ...
                    <div className="flex flex-col md:flex-row md:items-start gap-6">
                        <div className="w-32 pt-1 font-bold text-gray-800 text-lg shrink-0">重疾险</div>
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-8">
                            {renderInputGroup('重疾险补偿周期', config.ciMonths, 'ciMonths', '个月')}
                            {renderInputGroup('重疾险基础治疗费用', config.ciBaseCost, 'ciBaseCost', '万元')}
                        </div>
                    </div>
                    ...
                    {/* Section 2: Accident */}
                     <div className="flex flex-col md:flex-row md:items-start gap-6">
                        <div className="w-32 pt-1 font-bold text-gray-800 text-lg shrink-0">意外险</div>
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-8">
                            {renderInputGroup('意外险补偿周期', config.accMonths, 'accMonths', '个月')}
                        </div>
                    </div>
                    ...
                    {/* Section 3: Term Life */}
                    <div className="flex flex-col md:flex-row md:items-start gap-6">
                        <div className="w-32 pt-1 font-bold text-gray-800 text-lg shrink-0">定期寿险</div>
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-8">
                             {renderInputGroup('生活过渡期', config.tlTransitionMonths, 'tlTransitionMonths', '个月')}
                        </div>
                    </div>
                    ...
                    {/* Section 4: Pension */}
                    <div className="flex flex-col md:flex-row md:items-start gap-6">
                        <div className="w-32 pt-1 font-bold text-gray-800 text-lg shrink-0">养老金</div>
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-8">
                            {renderInputGroup('舒适养老护理费', config.comfortableNursingCost, 'comfortableNursingCost', '元/月')}
                            {renderInputGroup('舒适养老娱乐费', config.comfortableEntertainmentCost, 'comfortableEntertainmentCost', '元/月')}
                            {renderInputGroup('高端养老护理费加成系数', config.highEndNursingCostMultiplier, 'highEndNursingCostMultiplier', '倍')}
                            {renderInputGroup('高端养老娱乐费加成系数', config.highEndEntertainmentCostMultiplier, 'highEndEntertainmentCostMultiplier', '倍')}
                        </div>
                    </div>
                </div>
            </div>

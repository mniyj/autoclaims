...
const EditCompanyPage: React.FC<EditCompanyPageProps> = ({ companyCode, onBack }) => {
  ...
  useEffect(() => {
    const fetchCompany = async () => {
      try {
        ...
        // Try to find in list (which might be mixed InsuranceCompanyProfile and CompanyListItem if we were messy, 
        // but we assume list contains full profiles now or we search for code)

        // Note: detailed profiles might not be in the list if the list only returns summaries.
        // BUT our file based backend returns EVERYTHING. So it should be there.
        const found = list.find((c: any) => c.code === companyCode);
        if (found) {
          // If it's a legacy item (CompanyListItem only), it won't have basicInfo. 
          // We might need to handle that or assume data compliance.
          // For now, assume it matches InsuranceCompanyProfile or close enough.
          setProfile(found);
        }
      } catch (e) {
        console.error(e);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const list = await api.companies.list();
      const updatedList = list.map((c: any) => c.code === companyCode ? profile : c);
      await api.companies.saveAll(updatedList);
      alert('保险公司信息已更新！');
      onBack();
    } catch (error) {

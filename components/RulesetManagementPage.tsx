...
import {
  createRuleDraft,
  duplicateRuleset,
  normalizeRulesetStructure,
  publishRuleset,
  validateRuleset,
} from "./ruleset/workbenchUtils";

type DetailNavigationContext = {
  coverageCode?: string;
};

type RulesetNavigationPreset = {
  rulesetId?: string;
  productCode?: string;
  targetView?: "detail" | "validation";
};
...
const RulesetManagementPage: React.FC = () => {
  ...
  const [selectedRuleset, setSelectedRuleset] =
    useState<InsuranceRuleset | null>(null);
  const [isImportModalOpen, setIsImportModalOpen] = useState(false);
  const [isManualCreateModalOpen, setIsManualCreateModalOpen] = useState(false);
  const [loading, setLoading] = useState(true);
  const [presetSearchQuery, setPresetSearchQuery] = useState("");
  const [presetAutoOpenProductCode, setPresetAutoOpenProductCode] =
    useState("");
  const [detailNavigationContext, setDetailNavigationContext] =
    useState<DetailNavigationContext | null>(null);
  const [navigationPreset, setNavigationPreset] =
    useState<RulesetNavigationPreset | null>(null);
  ...
  useEffect(() => {
    ...
    const navigation = window.sessionStorage.getItem(
    ...
    if (focus) {
      try {
        setDetailNavigationContext(
          JSON.parse(focus) as DetailNavigationContext,
        );
      } catch (error) {
    ...
    if (navigation) {
      try {
        setNavigationPreset(JSON.parse(navigation) as RulesetNavigationPreset);
      } catch (error) {
        console.warn("Failed to parse ruleset navigation preset", error);
      } finally {
        window.sessionStorage.removeItem("ruleset_management_navigation");
      }
    }
  }, []);

  useEffect(() => {
    if (loading || currentView !== "list") return;
    const targetProductCode =
      navigationPreset?.productCode || presetAutoOpenProductCode;
    if (!targetProductCode && !navigationPreset?.rulesetId) return;

    const matched = navigationPreset?.rulesetId
      ? rulesets.find(
          (item) => item.ruleset_id === navigationPreset.rulesetId,
        ) ||
        (targetProductCode
          ? getPreferredRulesetForProduct(rulesets, targetProductCode)
          : null)
      : getPreferredRulesetForProduct(rulesets, targetProductCode || "");
    if (!matched) return;

    setSelectedRuleset(matched);
    setCurrentView(navigationPreset?.targetView || "detail");
    setPresetAutoOpenProductCode("");
    setNavigationPreset(null);
  }, [
    loading,
    presetAutoOpenProductCode,
    navigationPreset,
    rulesets,
    currentView,
  ]);
  ...
  useEffect(() => {
    const fetchRulesets = async () => {
      try {
        ...
        const normalizedRulesets = (rulesetData as InsuranceRuleset[]).map(
          (ruleset) =>
            normalizeRulesetStructure(
              ruleset,
              factCatalogData as FactCatalogField[],
            ),
        );
        setRulesets(normalizedRulesets);
        const hasSemanticGap = normalizedRulesets.some(
          (ruleset, index) =>
            JSON.stringify(ruleset.rules) !==
            JSON.stringify((rulesetData as InsuranceRuleset[])[index]?.rules),
        );
        if (hasSemanticGap) {
  ...
  const persistRulesets = async (nextRulesets: InsuranceRuleset[]) => {
    const synced = nextRulesets.map((ruleset) =>
      normalizeRulesetStructure(ruleset, factCatalog),
    );
  ...
  const handleImport = async (ruleset: InsuranceRuleset) => {
    ...
    if (existing) {
      ...
      newRulesets = rulesets.map((r) =>
        r.ruleset_id === ruleset.ruleset_id ? ruleset : r,
      );
    } else {
  ...
  const handleUpdateRuleset = async (updated: InsuranceRuleset) => {
    const newRulesets = rulesets.map((r) =>
      r.ruleset_id === updated.ruleset_id ? updated : r,
    );
    try {
      await persistRulesets(newRulesets);
  ...
  const handlePublish = async (ruleset: InsuranceRuleset) => {
    const issues = validateRuleset(ruleset).filter(
  ...
  const getPreviousRuleset = (ruleset: InsuranceRuleset | null) => {
    ...
    const sameProduct = rulesets
      .filter(
        (item) =>
          item.policy_info.product_code === ruleset.policy_info.product_code &&
          item.ruleset_id !== ruleset.ruleset_id,
      )
  ...
  return (
    <div>
      ...
      {currentView === "detail" && selectedRuleset && (
        <RulesetDetailView
          ...
          onCreateRule={(domain) => {
            ...
            const coverageCodes = selectedRuleset.policy_info.coverages
              .map((item) => item.coverage_code)
              .slice(0, 1);
            const newRule = createRuleDraft(
              {
                ...
                execution: {
                  domain,
                  loop_over:
                    domain === "ASSESSMENT" ? "claim.expense_items" : null,
                  item_alias: domain === "ASSESSMENT" ? "expense_item" : null,
                  item_action_on_reject:
                    domain === "ASSESSMENT" ? "ZERO_AMOUNT" : null,
                },
      ...
      <ManualRulesetModal
        ...
        onCreate={(draftRuleset) =>
          handleCreateManualRuleset(
            normalizeRulesetStructure(draftRuleset, factCatalog),
          )
        }
        products={products}

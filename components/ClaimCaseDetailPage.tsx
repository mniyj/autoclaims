        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {reviewResult && (
            <div className={`bg-white rounded-xl shadow-sm border-2 p-6 ${
              reviewResult.decision === 'APPROVE' ? 'border-green-200 bg-green-50/30' :
              reviewResult.decision === 'REJECT' ? 'border-red-200 bg-red-50/30' :
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-sm text-gray-500">耗时: {reviewResult.duration}ms</span>
              </div>
              ...
              <div className="grid grid-cols-3 gap-6 mb-6">
                <div>
                  ...
                  <p className={`text-lg font-bold ${
                    reviewResult.decision === 'APPROVE' ? 'text-green-600' :
                    reviewResult.decision === 'REJECT' ? 'text-red-600' :
                    'text-amber-600'
                  }`}>
                    {reviewResult.decision === 'APPROVE' ? '✅ 建议通过' :
                     reviewResult.decision === 'REJECT' ? '❌ 建议拒赔' :
                     '🔍 需人工复核'}
                  </p>
                </div>
                {reviewResult.amount !== null && (
                  <div>
                    ...
                    <p className="text-lg font-bold text-indigo-600">¥{reviewResult.amount.toLocaleString()}</p>
                  </div>
                )}
                {reviewResult.eligibility && (
                  <div>
                    <p className="text-sm text-gray-500 mb-1">责任判断</p>
                    <p className={`text-lg font-bold ${reviewResult.eligibility.eligible ? 'text-green-600' : 'text-red-600'}`}>
                      {reviewResult.eligibility.eligible ? '符合责任' : '不符合'}
                    </p>
                  </div>
                )}
              </div>

              {reviewResult.eligibility?.matchedRules && reviewResult.eligibility.matchedRules.length > 0 && (
                <div className="mb-4">
                  <p className="text-sm text-gray-500 mb-2">匹配规则</p>
                  <div className="flex flex-wrap gap-2">
                    {reviewResult.eligibility.matchedRules.map((rule, i) => (
                      <span key={i} className="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs font-mono">{rule}</span>
                    ))}
                  </div>
                </div>
              )}

...
import AnnuityCoverageEditor from './AnnuityCoverageEditor'
...
const AnnuityPlansEditor: React.FC<{ plans: Plan[]; onChange: (plans: Plan[]) => void }> = ({ plans, onChange }) => {
      ...
      {(plans || []).map((p, idx) => (
        <div key={idx} className="border border-gray-200 rounded-lg p-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input label="方案名称" id={`an-plan-${idx}`} name={`an-plan-${idx}`} value={p.planType || ''} onChange={e => updatePlan(idx, { planType: e.target.value })} placeholder="例如：标准版/增益版/尊享版" required />
          </div>
          <AnnuityCoverageEditor items={p.coverageDetails || []} onChange={(items) => updatePlan(idx, { coverageDetails: items })} />
...
export default AnnuityPlansEditor

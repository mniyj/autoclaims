...
const EditStrategyPage: React.FC<EditStrategyPageProps> = ({ strategy, onBack }) => {
    ...
    const handleSave = async (deployed: boolean) => {
        try {
            const updatedStrategy = { ...tableData, deployed };
            const list = await api.strategies.list();
            // Check if it exists
            const exists = list.some(s => s.code === updatedStrategy.code);
            let updatedList;
            if (exists) {
                updatedList = list.map(s => s.code === updatedStrategy.code ? updatedStrategy : s);
            } else {
                updatedList = [...list, updatedStrategy];
            }
            await api.strategies.saveAll(updatedList);
            alert(deployed ? '策略已发布！' : '草稿已保存！');
            onBack();
        } catch (error) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex justify-end space-x-3">
                <button onClick={() => handleSave(false)} className="px-5 py-2 bg-white text-gray-800 text-sm font-semibold rounded-md border border-gray-300 hover:bg-gray-50 transition">存草稿</button>
                ...
                <button onClick={() => handleSave(true)} className="px-5 py-2 bg-brand-blue-600 text-white text-sm font-semibold rounded-md shadow-sm hover:bg-brand-blue-700 transition">发布决策表</button>
            </div>

...
const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  const checkAuthStatus = async () => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL || "http://localhost:3008"}/api/dingtalk/login`,
      );
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL || "http://localhost:3008"}/api/dingtalk/callback?code=${code}&state=${state}`,
        );
        const data = await response.json();
        ...
        {loading ? (
          <div>
            ...
            <div style={{ marginTop: "24px" }}>
              ...
              <Paragraph type="secondary">请在钉钉应用中完成授权</Paragraph>
            </div>
          </div>
        ) : error ? (
          <Alert
          ...
          <div>
            ...
            <Paragraph type="secondary">即将进入理赔员工作台</Paragraph>
          </div>
        )}
      </Card>
    </div>

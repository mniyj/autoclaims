...
import { ClaimStatus, HistoricalClaim, Policy, DischargeSummaryData } from './types';
...
const MOCK_DISCHARGE_BRONCHITIS: DischargeSummaryData = {
  ...
  diagnoses: [
    ...
    { diagnosis_name: "咳嗽变异性哮喘?", diagnosis_type: "西医诊断", notes: "待排" }
  ],
...
const MOCK_DISCHARGE_DATA: DischargeSummaryData = {
  ...
  admission_details: {
    ...
    admission_condition_summary: "患者1天前无明显诱因出现脐周疼痛，后转移至右下腹。查体：右下腹压痛反跳痛阳性。",
  ...
  hospitalization_course_summary: "入院后完善相关检查，确诊为急性化脓性阑尾炎。于2024-02-21在全麻下行腹腔镜下阑尾切除术。术后给予抗感染、补液等对症治疗。患者切口愈合良好，体温正常，饮食恢复，予今日出院。",
  main_treatments_during_hospitalization: [
    { treatment_name: "腹腔镜下阑尾切除术", description: "2024-02-21 全麻下进行" },
    { treatment_name: "抗感染治疗", description: "头孢呋辛钠静滴" }
  ],
  condition_at_discharge: "一般情况良好，腹部切口愈合佳，无红肿渗出，腹平软，无压痛。",
  discharge_instructions: {
    medications: [
      { med_name: "头孢克肟胶囊", dosage: "0.1g", frequency: "每日2次", route: "口服", duration: "3天", notes: "饭后服用" }
    ],
    lifestyle_recommendations: ["清淡饮食", "避免剧烈运动1个月"],
    follow_up_appointments: [
      { date_or_interval: "术后1周", department: "普外科门诊", notes: "拆线/查看伤口" }
    ]
  },
  physician_info: {
...
export const MOCK_HISTORICAL_CLAIMS: HistoricalClaim[] = [
  { 
    ...
    documents: [
      { id: 'doc1', name: '驾驶证', type: 'image/jpeg', status: 'verified', category: '身份证件', ocrData: { name: '张建国' } },
      { id: 'doc2', name: '维修发票', type: 'image/jpeg', status: 'verified', category: '医疗发票', ocrData: { amount: 2500, merchant: '平安维修厂' } }
    ],
    timeline: [
      { date: '2024-01-10 09:30', label: '报案登记', description: '用户通过 AI 发起车辆报案', status: 'completed' },
      { date: '2024-01-10 10:15', label: '材料上传', description: '识别并关联驾驶证、发票', status: 'completed' },
      { date: '2024-01-11 14:00', label: '智能审核', description: 'AI 判定责任明确，符合条款', status: 'completed' },
      { date: '2024-01-12 16:45', label: '赔款拨付', description: '¥2500 已汇入用户尾号 8890 账户', status: 'completed' }
    ],
    assessment: {
      isLiable: true,
  ...
  { 
    ...
    documents: [
      { 
        ...
        analysis: {
          category: '出院小结',
          isRelevant: true,
          relevanceReasoning: '患者姓名与被保险人一致，住院时间在保单有效期内。',
          clarityScore: 98,
    ...
    timeline: [
      { date: '2024-02-20 18:00', label: '报案登记', description: '突发医疗报案，等待住院小结', status: 'completed' },
      { date: '2024-02-21 09:00', label: '材料上传', description: '用户已上传出院小结', status: 'completed' },
      { date: '2024-02-21 09:05', label: '智能审核', description: '已提取出院小结关键诊疗数据', status: 'completed' }
    ]
  },
    ...
    documents: [
      {
        ...
        analysis: {
          ...
          clarityScore: 90,
          completenessScore: 100,
          summary: '患者因双膝双手疼痛入院，确诊骨关节炎，给予药物及物理治疗后好转。',
          missingFields: [],
    ...
    documents: [
      {
        ...
        analysis: {
          ...
          summary: '患儿咳嗽3天，诊断为急性支气管炎，中西医结合治疗后治愈出院。',
          missingFields: [],
  ...
  { 
    ...
    documents: [
      { id: 'doc3', name: '受损部位照片', type: 'image/jpeg', status: 'verified', category: '事故照片' }
    ],
    timeline: [
      { date: '2024-03-05 11:20', label: '报案登记', description: '高速挡风玻璃受损报案', status: 'completed' },
      { date: '2024-03-05 11:45', label: '影像识别', description: 'AI 正在分析玻璃破损程度', status: 'active' }
    ]
  },
  { 
    id: 'CLM-2024-004', 
    ...
    documents: [
      { id: 'doc4', name: '物业证明', type: 'image/jpeg', status: 'verified', category: '事故证明' }
    ],
    timeline: [
      { date: '2024-03-15 14:00', label: '报案登记', description: '财产损失报案', status: 'completed' },
      { date: '2024-03-15 15:30', label: '智能审核', description: 'AI 已完成定损分析', status: 'completed' },
      { date: '2024-03-16 09:00', label: '等待支付', description: '财务处理中', status: 'active' }
    ],
    assessment: {
      isLiable: true,
  ...
  { 
    ...
    documents: [
      { id: 'doc5', name: '登机牌截图', type: 'image/jpeg', status: 'verified', category: '发票' }
    ],
    timeline: [
      { date: '2024-04-01 10:00', label: '报案登记', description: '误机补偿申请', status: 'completed' },
      { date: '2024-04-01 10:30', label: 'AI 核赔', description: '检测到拒赔动因', status: 'completed' },
      { date: '2024-04-01 10:35', label: '结论下达', description: '理赔被拒绝', status: 'completed' }
    ],
    assessment: {
      isLiable: false,

    ...
    const worker = async () => {
      while (true) {
        ...
        try {
          ...
          results[current] = { ...att, analysis: mappedAnalysis, status: 'success', url: analysis.ossUrl };
        } catch (err) {
                              ...
                              <>
                                  {visiblePolicies.map(p => {
                                      ...
                                      return (
                                        <Card 
                                            key={p.id} 
                                            onClick={() => !isExpired && handlePolicySelect(p)}
                                            className={`relative overflow-hidden transition-all ${isExpired ? 'grayscale opacity-75 cursor-not-allowed bg-slate-50' : 'hover:shadow-md cursor-pointer'}`}
                                        >
                 ...
                 <div className="p-5 bg-gradient-to-br from-blue-50 to-white rounded-2xl border border-blue-100 shadow-sm">
                    ...
                    <div className="flex items-center justify-between mb-3">
                       ...
                       <span className="text-2xl text-blue-500">
                         <i className={`fas ${
                           selectedDetailClaim.status === ClaimStatus.PAID ? 'fa-circle-check' : 
                        ...
                        <div className="space-y-6">
                          {selectedDetailClaim.timeline.map((event, idx) => (
                            <div key={idx} className="relative flex gap-4 group">
                               <div className={`w-4 h-4 rounded-full mt-1 shrink-0 z-10 ring-4 ring-white transition-all duration-500 ${
                                 event.status === 'completed' ? 'bg-blue-500 shadow-lg shadow-blue-500/30' :

...
const AUTO_COVERAGE_ALIASES = {
  ...
  [AUTO_COVERAGE_CODES.THIRD_PARTY]: [
    "第三者责任险",
    "第三者责任保险",
    "THIRD_PARTY",
    "TPL",
  ],
  [AUTO_COVERAGE_CODES.VEHICLE_DAMAGE]: [
    "车辆损失险",
    "机动车损失保险",
    "VEHICLE_DAMAGE",
    "VHL",
  ],
  [AUTO_COVERAGE_CODES.DRIVER_PASSENGER]: [
    "车上人员责任险",
    "驾乘人员责任险",
    "DRIVER_PASSENGER",
    "SEAT",
...
export function getAutoFaultRatio(context) {
  return (
    ...
    normalizeRatio(
      context.claim?.claimant_liability_pct !== undefined
        ? 100 - context.claim.claimant_liability_pct
        : null,
    ) ??
    normalizeRatio(
      context.claim?.claimantLiabilityPct !== undefined
        ? 100 - context.claim.claimantLiabilityPct
        : null,
    ) ??
    1
  );
...
export function getAutoCoverageConfig(
  productCode,
  coverageCode,
...
export function getAutoLossAmount(
  context,
  factResult,
  coverageCode,
  coverageConfig = null,
  ...
  if (coverageCode === AUTO_COVERAGE_CODES.DRIVER_PASSENGER) {
    const injuryGrade =
      context.claim?.injury_grade || context.claim?.injuryGrade;
    const perSeatSumInsured = getConfiguredAmount(
      coverageConfig?.per_seat_sum_insured,
    );

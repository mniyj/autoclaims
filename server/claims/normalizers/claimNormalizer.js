...
export function normalizeClaimContext(
  claimCase = {},
  ocrData = {},
  invoiceItems = [],
) {
  ...
  normalized.expense_items =
    invoiceItems.length > 0 ? invoiceItems : safeOcrData.chargeItems || [];
  if (normalized.expense_items.length === 0) {
    const socialMedicalAmount =
      toNumber(
  ...
  normalized.total_claimed_amount = calculateClaimedTotal(
    normalized.expense_items,
  );
  ...
  normalized.is_drunk_driving = toBooleanLike(
  ...
  normalized.insured_age = extractAgeLike(
  ...
  normalized.fault_ratio = toNumber(
  ...
  normalized.insured_liability_ratio = toNumber(
  ...
  normalized.third_party_liability_ratio = toNumber(
  ...
  normalized.claimant_liability_pct = toNumber(
  ...
  normalized.third_party_loss_amount = toNumber(
  ...
  normalized.third_party_property_damage_amount = toNumber(
  ...
  normalized.third_party_injury_amount = toNumber(
  ...
  normalized.third_party_death_disability_amount = toNumber(
  ...
  normalized.vehicle_damage_amount = toNumber(
  ...
  normalized.passenger_injury_amount = toNumber(
  ...
  if (safeOcrData.insurancePayment) {
    normalized.social_insurance_paid =
      safeOcrData.insurancePayment.governmentFundPayment || 0;
    normalized.personal_payment =
      safeOcrData.insurancePayment.personalPayment || 0;
    normalized.personal_self_pay =
      safeOcrData.insurancePayment.personalSelfPayment || 0;
    normalized.personal_self_expense =
      safeOcrData.insurancePayment.personalSelfExpense || 0;
  }
  ...
  const booleanFactDefaults = [
    ...
    "illegal_drug_use",
    "illegal_driving",
    // pre_existing_condition 不在此列——缺失时应保持 null 以触发 IS_NULL 规则（ZAOPD108M）转人工，
    // 而非默认 false 绕过既往症检查。其值由 preExistingConditionAssessor 填充。

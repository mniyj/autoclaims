...
import {
  getFieldValue,
  evaluateConditions as evaluateConditionsImported,
  ...
  switch (action_type) {
    ...
    case "SET_CLAIM_RATIO":
      ...
      if (ratio === undefined && Array.isArray(params.disability_grade_table)) {
        ...
        const matchedRatio = params.disability_grade_table.find(
          (item) => Number(item.grade) === disabilityGrade,
        );
    ...
    case "ROUTE_CLAIM_MANUAL":
      ...
      result.data.intervention_required = true;
      result.data.intervention_type = "RULE_MANUAL_ROUTE";
    ...
    case "APPLY_CAP":
      ...
      const currentAmount =
        resolveStateField(state, capField) ??
        getFieldValue(context, capField) ??
    ...
    case "APPLY_DEDUCTIBLE":
      ...
      const amountAfterDeductible = Math.max(
        0,
        amountBeforeDeductible - deductible,
      );
    ...
    case "SUM_COVERAGES":
      ...
      const totalSumInsured = coverages.reduce(
        (sum, c) => sum + (c.sum_insured || 0),
        0,
      );
...
export function executeItemLoopAction(rule, context, state) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    // 评估条件并执行动作（导入在文件顶部）
    const conditionMet = evaluateConditionsImported(
      rule.conditions,
      itemContext,
    );

...
import {
  getLatestMaterialValidationResults,
  getRuleset,
  buildContext,
} from "./context.js";
import { runPreProcessors } from "../claims/preProcessorRunner.js";
  ...
  executionDetails.forEach((item) => {
    if (item?.rule_id && item?.rule_name) {
      pushRule({
        ...
        action: item.action_result
          ? { action_type: item.action_result.action_type || null }
          : null,
...
function buildMatchedRuleDetails({ context, eligibilityResult, amountResult }) {
  ...
  (eligibilityResult?.matchedRules || []).forEach((ruleId) => {
    const executionDetail = (eligibilityResult?.executionDetails || []).find(
      (item) => item.rule_id === ruleId,
    );
    register(ruleId, {
      domain: "LIABILITY",
      effect:
        executionDetail?.action_result?.message ||
        executionDetail?.action_result?.action_type,
  ...
  (amountResult?.executionDetails || []).forEach((item) => {
    ...
    if (
      Array.isArray(item?.item_results) &&
      item.item_results.some((result) => result?.conditionMet)
    ) {
      register(item.rule_id, {
        domain: "SETTLEMENT",
        effect:
          item.action_result?.message ||
          item.action_result?.action_type ||
          "费用项规则命中",
      });
    }
  });
...
export async function checkEligibility({
  ...
  // 执行配置驱动的前处理器（取代硬编码的 enrichPreExistingFact）
  const { ocrData: enrichedOcrData, assessments: preProcessorResults } =
  ...
  const preExistingAssessment = preProcessorResults.find(
...
export async function calculateAmount({
  ...
  logRuleExecution({
    rulesetId:
      result.context?.ruleset_id || eligibilityResult?.context?.ruleset_id,
    claimCaseId,
    productCode,
 ...
 */
export async function executeFullReview({
  claimCaseId,
  productCode,
  ocrData = {},
  invoiceItems = [],
  ...
  const settlementDecision =
    amountResult.settlementDecision ||
    (amountResult.finalAmount > 0 ? "PAY" : "ZERO_PAY");
  const coverageResults =
    amountResult.coverageResults ||
    (amountResult.coverageResult ? [amountResult.coverageResult] : []);
  const materialCompleteness = evaluateMaterialCompleteness({
    claimCaseId,
    productCode,
    claimType:
      amountResult.claimType || eligibilityResult.context?.product_line,
    coverageCode: amountResult.coverageCode,
  });
  const materialValidationResults =
    getLatestMaterialValidationResults(claimCaseId);
      ...
      ? [
          {
            ...
            metadata: {
              missingMaterials: materialCompleteness.missingMaterials.map(
                (item) => item.name,
              ),
            },
          },
  ...
  const reasonCodes = [
    ...(eligibilityResult.rejectionReasons || []).map(
  ...
  const finalDecision =
    ...
    amountResult.needsManualReview ||
    hasMissingMaterials ||
    hasMaterialValidationFailure
      ? "MANUAL_REVIEW"
      : eligibilityResult.eligible
        ? "APPROVE"
        : "REJECT";
  const finalIntakeDecision = hasMissingMaterials ? "PENDING_MATERIAL" : "PASS";
  const finalLiabilityDecision =
    hasMissingMaterials || hasMaterialValidationFailure
      ? "MANUAL_REVIEW"
      : liabilityDecision;
  const finalAssessmentDecision =
    (hasMissingMaterials || hasMaterialValidationFailure) &&
    assessmentDecision === "ASSESSED"
      ? "PARTIAL_ASSESSED"
      : assessmentDecision;
  const finalSettlementDecision =
    hasMissingMaterials || hasMaterialValidationFailure
      ? "MANUAL_REVIEW"
      : settlementDecision;
  ...
  return {
    ...
    claimType:
      amountResult.claimType ||
      eligibilityResult.context?.product_line ||
      "UNKNOWN",
    coverageResults,
    reasonCodes,
    warnings,
    ...
    eligibility: {
      ...
      preExistingAssessment: eligibilityResult.preExistingAssessment || null,
    },

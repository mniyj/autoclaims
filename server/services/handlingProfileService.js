...
function includesAnyText(target, candidates = []) {
  ...
  return (candidates || []).some((candidate) => normalizedTarget.includes(normalizeText(candidate)));
}

function buildProfileContext({ claimCase = null, aggregation = null, materials = [] } = {}) {
  ...
  const recognizedPayment = Number(aggregation?.paymentSummary?.confirmedPaidAmount || 0);
  ...
  const insuranceType = claimCase?.insuranceType || claimCase?.productName || "";
  const accidentReason = claimCase?.accidentReason || claimCase?.selectedAccidentCauseId || "";
  ...
  return {
    ...
    hasLiabilityEvidence:
      ...
      ["mat-57", "mat-58", "mat-64", "mat-66"].some((id) => materialIds.has(id)),
    ...
    hasIdentityChainEvidence: identityChainEvidenceCount > 0 || materialIds.has("mat-64"),
    ...
    matchesLiabilityReason: includesAnyText(insuranceType, ["责任", "机械", "三者", "雇主"]) ||
...
function matchesTriggers(profile, context) {
  ...
  if (typeof triggers.deathConfirmed === "boolean" && Boolean(context.deathConfirmed) !== triggers.deathConfirmed) {
    return false;
  }

  if (Array.isArray(triggers.requiresAll) && triggers.requiresAll.some((flag) => !checkFlag(context, flag))) {
    return false;
  }

  if (Array.isArray(triggers.requiresAny) && triggers.requiresAny.length > 0) {
    const matched = triggers.requiresAny.some((flag) => checkFlag(context, flag));
...
export function resolveHandlingProfile({ claimCase = null, aggregation = null, materials = [] } = {}) {
  const profiles = loadProfiles()
    .slice()
    .sort((a, b) => Number(b.priority || 0) - Number(a.priority || 0));
  const context = buildProfileContext({ claimCase, aggregation, materials });
  const matched = profiles.find((profile) => matchesTriggers(profile, context)) || null;
...
export function buildProfileReviewTasks({ profile = null, context = null } = {}) {
...
export function summarizeHandlingProfile({ claimCase = null, aggregation = null, materials = [] } = {}) {
  const { context, profile } = resolveHandlingProfile({ claimCase, aggregation, materials });

...
import {
  VoiceStateMachine,
  VoiceSessionState,
} from "./state/VoiceSessionState.js";
...
export class VoiceSession {
  ...
  private async handleAudioMessage(payload: any): Promise<void> {
    ...
    console.log(
      `[VoiceSession] Received audio chunk, isFinal: ${payload.isFinal}, buffer size: ${this.audioBuffer.length}`,
    );
    ...
    if (payload.data) {
      ...
      console.log(
        `[VoiceSession] Audio buffer now has ${this.audioBuffer.length} chunks`,
      );
    }

    if (payload.isFinal && this.audioBuffer.length > 0) {
      console.log(
        `[VoiceSession] Processing final audio, total chunks: ${this.audioBuffer.length}`,
      );
  ...
  private async processUserInput(text: string): Promise<void> {
    ...
    if (
      currentState === VoiceSessionState.PROCESSING ||
    ...
    try {
      ...
      if (missingRequiredSlots.length > 0) {
        // 询问缺失的必填槽位
        const slotDef = CLAIM_REPORT_SLOTS.find(
          (s) => s.name === missingRequiredSlots[0],
        );
        if (slotDef) {
          const question = this.generateSlotQuestion(slotDef);
          await this.sendResponse(question);
      ...
      if (toolCall) {
        ...
        this.stateMachine.addToHistory(
          "system",
  ...
  private async generateToolResponse(
    ...
    switch (toolName) {
      ...
      case "提交报案":
        ...
        const materials = claim.requiredMaterials
          ?.map((m: any) => m.name)
          .join("、");

...
export const MOCK_CLAIMS_MATERIALS: ClaimsMaterial[] = [
  ...
  // ══════════════════════════════════════════════════
  {
    id: 'mat-1',
    name: '身份证正面',
    description: '被保险人或受益人的身份证正面照片，包含姓名、性别、民族、出生日期、住址、身份证号等信息。',
    ...
    required: true,
    aiAuditPrompt: '请从身份证正面提取姓名、性别、民族、出生日期、住址和身份证号码。校验要点：1）身份证号码必须为18位，校验最后一位校验码是否正确；2）比对姓名是否与保单上的被保险人/受益人一致；3）检查出生日期与身份证号中的出生日期编码是否一致。若有信息模糊无法识别，请标注具体字段。'
  },
  {
    id: 'mat-2',
    name: '身份证反面',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        valid_from: { type: 'string', format: 'date', description: '有效期起始日期' },
        valid_until: { type: 'string', description: '有效期截止日期（可能为"长期"）' }
      },
      required: ['valid_from', 'valid_until']
    }, null, 2),
    required: true,
    aiAuditPrompt: '请从身份证反面提取签发机关、有效期起始日期和截止日期。校验要点：1）有效期截止日期是否晚于事故发生日/就诊日期，若已过期则标记为"证件已过期"；2）若截止日期为"长期"则视为有效。'
  },
  {
    id: 'mat-3',
    name: '驾驶证（含副页）',
    description: '机动车驾驶人的驾驶证正页及副页，用于交通事故理赔时核实驾驶资质。',
    jsonSchema: JSON.stringify({
      type: 'object',
      properties: {
        name: { type: 'string', description: '姓名' },
        ...
        issue_date: { type: 'string', format: 'date', description: '初次领证日期' },
        file_number: { type: 'string', description: '档案编号' }
      },
      required: ['name', 'license_number', 'vehicle_class']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请从驾驶证正页和副页提取姓名、驾驶证号、准驾车型、有效期限和初次领证日期。校验要点：1）驾驶证有效期是否覆盖事故发生日期；2）准驾车型是否与事故中驾驶的车辆类型匹配（如C1不能驾驶大型车辆）；3）是否存在扣分或违章记录标注。'
  },
  {
    id: 'mat-4',
    name: '行驶证（含副页）',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        register_date: { type: 'string', format: 'date', description: '注册日期' },
        inspection_valid_until: { type: 'string', format: 'date', description: '检验有效期至' }
      },
      required: ['plate_number', 'vehicle_type', 'owner']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请从行驶证正页和副页提取号牌号码、车辆类型、所有人、VIN码、发动机号、注册日期和检验有效期。校验要点：1）检验有效期是否覆盖事故发生日期，若已过期则标记；2）车辆类型与驾驶证准驾车型是否匹配。'
  },
  {
    id: 'mat-5',
    name: '委托授权书',
    ...
    required: false,
    aiAuditPrompt: '请从委托授权书提取委托人姓名和身份证号、被委托人姓名和身份证号、委托事项范围和日期。校验要点：1）委托人是否为保单被保险人或受益人；2）授权范围是否明确覆盖理赔申请及领取赔款事项；3）是否有委托人亲笔签名或指纹确认；4）委托日期是否合理（不应早于事故日期）。'
  },
  {
    id: 'mat-6',
    name: '银行卡（正反面）',
    ...
    required: true,
    aiAuditPrompt: '请从银行卡照片提取开户银行名称、银行卡号和持卡人姓名。校验要点：1）持卡人姓名是否与被保险人或受益人一致（若有委托授权书则应与被委托人一致）；2）银行卡号格式是否合法（一般为16-19位数字）；3）确认银行卡号清晰可辨。'
  },
  {
    id: 'mat-7',
    name: '户籍所在地证明',
    description: '户籍所在地派出所或社区出具的户籍证明，用于确认城镇/农村户籍性质以适用不同赔偿标准。',
    jsonSchema: JSON.stringify({
      type: 'object',
      properties: {
        name: { type: 'string', description: '姓名' },
        ...
        household_type: { type: 'string', description: '户籍性质（城镇/农村）' },
        issue_date: { type: 'string', format: 'date', description: '开具日期' },
        issuing_authority: { type: 'string', description: '出具机关' }
      },
      required: ['name', 'household_address', 'household_type']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请提取户籍证明中的姓名、身份证号、户籍地址、户籍性质（城镇或农村）、开具日期和出具机关。校验要点：1）户籍性质直接影响残疾赔偿金和死亡赔偿金计算标准（城镇标准通常高于农村标准）；2）证明是否由户籍所在地公安派出所或社区出具并盖章。'
  },
  ...
  // ══════════════════════════════════════════════════
  {
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        accident_date: { type: 'string', format: 'date', description: '事故发生日期' },
        ...
        parties: {
          ...
          items: {
            ...
            properties: {
              ...
              responsibility: { type: 'string', description: '责任认定（全责/主责/同责/次责/无责）' }
            }
          },
    ...
    required: false,
    aiAuditPrompt: '请从交通事故责任认定书中提取文书编号、事故日期时间、地点、各方当事人信息和责任划分、事故经过摘要。校验要点：1）明确各方责任比例（全责100%、主责70%、同责50%、次责30%、无责0%）；2）事故日期应在保险有效期内；3）被保险人在事故中的角色和责任。'
  },
  {
    id: 'mat-9',
    name: '认定工伤决定书',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        injury_date: { type: 'string', format: 'date', description: '受伤日期' },
        injury_description: { type: 'string', description: '受伤经过及诊断' },
        work_injury_conclusion: { type: 'string', description: '工伤认定结论' },
        decision_date: { type: 'string', format: 'date', description: '决定日期' },
        issuing_authority: { type: 'string', description: '出具机关' }
      },
      required: ['employee_name', 'employer', 'injury_date', 'work_injury_conclusion']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请从认定工伤决定书中提取决定书编号、职工姓名、用人单位、受伤日期、受伤经过及诊断、工伤认定结论和决定日期。校验要点：1）确认工伤认定结论为"认定为工伤"或"视同工伤"；2）受伤日期与就诊日期逻辑一致；3）确认出具机关为人力资源和社会保障局。'
  },
  {
    id: 'mat-10',
    name: '工伤认定申请表',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        injury_date: { type: 'string', format: 'date', description: '事故伤害日期' },
        injury_cause: { type: 'string', description: '事故伤害原因' },
        injury_description: { type: 'string', description: '受伤害经过简述' },
        application_date: { type: 'string', format: 'date', description: '申请日期' }
      },
      required: ['applicant_name', 'employer', 'injury_date']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请从工伤认定申请表中提取申请人姓名、用人单位、事故伤害日期、伤害原因、受伤经过简述和申请日期。校验要点：1）申请人信息与认定工伤决定书中的职工信息应一致；2）事故日期和伤害经过描述应与工伤决定书吻合。'
  },
  ...
  // ══════════════════════════════════════════════════
  {
    ...
    required: false,
    aiAuditPrompt: '请从门（急）诊病历中提取医院名称、就诊科室、就诊日期、患者姓名、主诉、现病史、诊断结果、治疗方案和医生姓名。校验要点：1）就诊日期与事故发生日期的时间逻辑是否合理（应在事故发生后）；2）诊断结果与事故伤情是否存在因果关系；3）治疗方案是否与诊断匹配。'
  },
  {
    id: 'mat-12',
    name: '住院病历/病案',
    ...
    jsonSchema: JSON.stringify({
      ...
      required: ['hospital_name', 'admission_date', 'discharge_date', 'discharge_diagnosis']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请从住院病历中提取医院名称、患者姓名、入院日期、出院日期、住院天数、入院诊断、出院诊断、治疗经过和出院医嘱。校验要点：1）住院天数计算是否正确（出院日期-入院日期）；2）入院诊断与出院诊断是否一致或有合理变化；3）治疗经过与诊断是否匹配。'
  },
  {
    id: 'mat-13',
    name: '诊断证明书',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        diagnosis_date: { type: 'string', format: 'date', description: '诊断日期' },
    ...
    required: true,
    aiAuditPrompt: '请从诊断证明书中提取医院名称、患者姓名、诊断结果、诊断日期、诊断医生、治疗建议和建议休息天数。校验要点：1）诊断结果与事故伤情是否存在因果关系；2）医院是否为二级及以上医疗机构；3）是否有医院诊断专用章；4）建议休息天数与伤情严重程度是否匹配。若信息缺失，请指出具体缺失项。'
  },
  {
    id: 'mat-14',
    name: '转院证明',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        transfer_date: { type: 'string', format: 'date', description: '转院日期' },
        transfer_reason: { type: 'string', description: '转院原因' }
      },
      required: ['original_hospital', 'receiving_hospital', 'transfer_date', 'transfer_reason']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请提取转院证明中的患者姓名、转出医院、转入医院、转院日期和转院原因。校验要点：1）转院原因是否合理（如原医院不具备治疗条件）；2）转出和转入医院是否均为正规医疗机构；3）转院日期与住院记录是否衔接。'
  },
  {
    id: 'mat-15',
    name: '医嘱休息证明（病假单）',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        patient_name: { type: 'string', description: '患者姓名' },
        diagnosis: { type: 'string', description: '诊断' },
        rest_start_date: { type: 'string', format: 'date', description: '休息起始日期' },
        rest_end_date: { type: 'string', format: 'date', description: '休息截止日期' },
        rest_days: { type: 'number', description: '建议休息天数' },
        doctor_name: { type: 'string', description: '医生姓名' }
      },
    ...
    required: false,
    aiAuditPrompt: '请提取医嘱休息证明中的医院名称、患者姓名、诊断、休息起止日期和休息天数。校验要点：1）建休天数是否与伤情严重程度匹配（轻微伤一般不超过15天，骨折通常3-6个月）；2）休息期与住院期是否有重叠（住院期间已包含在误工期内）；3）是否有医生签名和医院盖章。'
  },
  {
    id: 'mat-16',
    name: '医嘱护理证明',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        hospital_name: { type: 'string', description: '医院名称' },
        patient_name: { type: 'string', description: '患者姓名' },
        nursing_level: { type: 'string', description: '护理等级（特级/一级/二级/三级）' },
        nursing_period_start: { type: 'string', format: 'date', description: '护理期开始日期' },
        nursing_period_end: { type: 'string', format: 'date', description: '护理期结束日期' },
        nursing_persons: { type: 'number', description: '护理人数' },
        doctor_name: { type: 'string', description: '医生姓名' }
      },
      required: ['hospital_name', 'patient_name', 'nursing_level', 'nursing_persons']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请提取医嘱护理证明中的医院名称、患者姓名、护理等级、护理期限和护理人数。校验要点：1）护理等级与伤情严重程度是否匹配；2）护理人数一般为1人，2人护理需要特殊医嘱说明；3）护理期限是否合理。'
  },
  {
    id: 'mat-17',
    name: '营养费医嘱',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        hospital_name: { type: 'string', description: '医院名称' },
        patient_name: { type: 'string', description: '患者姓名' },
        nutrition_period_start: { type: 'string', format: 'date', description: '营养期开始日期' },
        nutrition_period_end: { type: 'string', format: 'date', description: '营养期结束日期' },
    ...
    required: false,
    aiAuditPrompt: '请提取营养费医嘱中的医院名称、患者姓名、营养期限和天数。校验要点：1）营养期天数是否有医嘱明确支持；2）营养期限与住院/恢复期是否合理衔接；3）是否有医生签名。'
  },
  {
    id: 'mat-18',
    name: '住院天数证明',
    description: '医院出具的住院天数证明文件，用于核实住院期间的各项费用计算基础。',
    jsonSchema: JSON.stringify({
      type: 'object',
      properties: {
        hospital_name: { type: 'string', description: '医院名称' },
      ...
      required: ['hospital_name', 'patient_name', 'admission_date', 'discharge_date', 'total_days']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请提取住院天数证明中的医院名称、患者姓名、入院日期、出院日期、住院总天数和科室。校验要点：1）住院天数计算是否正确（出院日期-入院日期）；2）与住院病历中的入院和出院日期是否一致。'
  },
  {
    id: 'mat-19',
    name: '外购药处方笺',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        prescription_date: { type: 'string', format: 'date', description: '处方日期' }
      },
      required: ['hospital_name', 'patient_name', 'drug_name', 'prescribing_doctor']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请提取外购药处方笺中的开方医院、患者姓名、药品名称、用法用量、数量、处方医生和处方日期。校验要点：1）外购药品是否与诊断结果相关；2）处方是否由主治医师开具；3）处方日期是否在治疗期间内；4）与购药发票中的药品名称和数量是否一致。'
  },
  ...
  // ══════════════════════════════════════════════════
  {
    id: 'mat-20',
    name: '医疗费发票',
    description: '医院出具的医疗费用发票（门诊或住院发票），是理赔金额核定的主要依据。',
    jsonSchema: JSON.stringify({
      type: 'object',
      properties: {
        hospital_name: { type: 'string', description: '医院名称' },
    ...
    required: true,
    aiAuditPrompt: '请从医疗费发票中提取医院名称、患者姓名、发票号码、发票代码、开票日期、金额合计、个人自付、医保统筹支付和个人账户支付。校验要点：1）各项金额相加是否等于合计金额；2）发票日期是否在就诊期间内；3）发票是否为正规财政票据或税务发票；4）与费用明细清单中的总金额是否一致。'
  },
  {
    id: 'mat-21',
    name: '费用明细清单',
    ...
    required: true,
    aiAuditPrompt: '请从费用明细清单中提取医院名称、患者姓名、所有费用明细项（项目名称、类别、数量、单价、金额）和费用总额。校验要点：1）各项费用金额=单价×数量；2）所有项目金额之和是否等于总额；3）与医疗费发票的总金额是否一致；4）是否存在异常高价项目。'
  },
  {
    id: 'mat-22',
    name: '购药发票',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        invoice_number: { type: 'string', description: '发票号码' },
        invoice_date: { type: 'string', format: 'date', description: '购药日期' },
        drug_items: {
          type: 'array',
          items: {
            type: 'object',
    ...
    required: false,
    aiAuditPrompt: '请从购药发票中提取药房名称、发票号码、购药日期、药品明细（名称、数量、单价、金额）和合计金额。校验要点：1）购买的药品是否与外购药处方笺中开具的药品一致；2）药品数量是否与处方匹配；3）购药日期是否在治疗期间内；4）药房是否为正规药店。'
  },
  {
    id: 'mat-23',
    name: '辅助器具发票',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        invoice_number: { type: 'string', description: '发票号码' },
        invoice_date: { type: 'string', format: 'date', description: '购买日期' },
    ...
    required: false,
    aiAuditPrompt: '请从辅助器具发票中提取销售商名称、发票号码、器具名称、规格型号、数量、单价和合计金额。校验要点：1）器具类型是否与伤残等级和伤情匹配（如截肢才需要假肢）；2）价格是否在当地辅助器具配制机构的标准范围内；3）是否有残疾辅助器具证明或医嘱支持。'
  },
  {
    id: 'mat-24',
    name: '护理费发票',
    ...
    jsonSchema: JSON.stringify({
      type: 'object',
      properties: {
        nursing_institution: { type: 'string', description: '护理机构/护工姓名' },
        invoice_number: { type: 'string', description: '发票号码' },
        invoice_date: { type: 'string', format: 'date', description: '发票日期' },
        nursing_period_start: { type: 'string', format: 'date', description: '护理起始日期' },
        nursing_period_end: { type: 'string', format: 'date', description: '护理结束日期' },
    ...
    required: false,
    aiAuditPrompt: '请从护理费发票中提取护理机构/护工信息、发票号码、护理起止日期、护理天数、日费率和合计金额。校验要点：1）护理天数是否与医嘱护理证明中的护理期限一致；2）日费率是否在当地护理行业合理范围内；3）金额=天数×日费率的计算是否正确。'
  },
  {
    id: 'mat-25',
    name: '交通费票据',
    ...
    jsonSchema: JSON.stringify({
      type: 'object',
      properties: {
        transport_type: { type: 'string', description: '交通方式（出租车/公交/地铁/火车/飞机等）' },
    ...
    required: false,
    aiAuditPrompt: '请从交通费票据中提取交通方式、出发地、目的地、日期、金额和票号。校验要点：1）交通日期是否在就医期间内（与诊断证明、住院病历日期对应）；2）出发地或目的地是否与就诊医院所在地一致；3）交通方式是否合理（一般应选择经济便捷方式，特殊情况如伤情严重可乘坐出租车/飞机）。'
  },
  {
    id: 'mat-26',
    name: '住宿费发票',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        invoice_number: { type: 'string', description: '发票号码' },
        check_in_date: { type: 'string', format: 'date', description: '入住日期' },
        check_out_date: { type: 'string', format: 'date', description: '退房日期' },
    ...
    required: false,
    aiAuditPrompt: '请从住宿费发票中提取住宿单位名称、发票号码、入住退房日期、天数、日房价和合计金额。校验要点：1）住宿日期是否与就医日期匹配（应为异地就医期间）；2）住宿地点是否在就诊医院附近；3）日房价是否在当地合理标准内（一般参照当地国家机关一般工作人员出差住宿标准）。'
  },
  {
    id: 'mat-27',
    name: '鉴定费发票',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        invoice_number: { type: 'string', description: '发票号码' },
        invoice_date: { type: 'string', format: 'date', description: '发票日期' },
        appraisal_type: { type: 'string', description: '鉴定类型（伤残鉴定/劳动能力鉴定/护理依赖鉴定等）' },
        total_amount: { type: 'number', description: '鉴定费金额' }
      },
      required: ['institution_name', 'appraisal_type', 'total_amount']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请从鉴定费发票中提取鉴定机构名称、发票号码、日期、鉴定类型和金额。校验要点：1）鉴定机构是否具有法定资质；2）鉴定费用是否在当地收费标准范围内；3）鉴定类型与案件所需鉴定项目是否匹配。'
  },
  {
    id: 'mat-28',
    name: '丧葬费票据',
    ...
    required: false,
    aiAuditPrompt: '请从丧葬费票据中提取殡葬服务单位、发票信息、服务项目明细和合计金额。校验要点：1）丧葬费法定标准为受诉法院所在地上一年度职工月平均工资×6个月，实际费用仅作参考；2）服务项目是否合理（基本丧葬服务）。'
  },
  ...
  // ══════════════════════════════════════════════════
  {
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        absence_start_date: { type: 'string', format: 'date', description: '误工起始日期' },
        absence_end_date: { type: 'string', format: 'date', description: '误工截止日期' },
    ...
    required: false,
    aiAuditPrompt: '请从误工证明中提取用人单位名称、员工姓名、职务、误工起止日期、误工天数和扣发工资金额。校验要点：1）误工期是否与医嘱建休期匹配（误工期不超过建休期+住院期）；2）是否有用人单位公章；3）扣发工资金额是否与收入证明中的收入水平一致。'
  },
  {
    id: 'mat-30',
    name: '收入证明/工资证明',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        employment_date: { type: 'string', format: 'date', description: '入职日期' },
        has_employer_seal: { type: 'boolean', description: '是否有单位公章' }
      },
      required: ['employer_name', 'employee_name', 'monthly_salary']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请从收入证明中提取用人单位、员工姓名、职务、月工资和年收入。校验要点：1）月收入水平是否与行业及地区平均水平大致匹配，过高需要银行流水和纳税证明佐证；2）年收入是否约等于月工资×12；3）是否有用人单位公章。'
  },
  {
    id: 'mat-31',
    name: '劳动合同',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        contract_start_date: { type: 'string', format: 'date', description: '合同起始日期' },
        contract_end_date: { type: 'string', format: 'date', description: '合同终止日期' },
        position: { type: 'string', description: '工作岗位' },
        salary: { type: 'number', description: '劳动报酬（月薪）' },
        contract_type: { type: 'string', description: '合同类型（固定期限/无固定期限/以完成一定工作任务为期限）' }
      },
      required: ['employer_name', 'employee_name', 'contract_start_date']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请从劳动合同中提取用人单位、劳动者姓名、合同期限、工作岗位、劳动报酬和合同类型。校验要点：1）合同有效期是否覆盖事故发生日期（证明事故时存在劳动关系）；2）合同中约定的工资是否与收入证明一致；3）用人单位是否与误工证明中的单位一致。'
  },
  {
    id: 'mat-32',
    name: '工资银行流水',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        bank_name: { type: 'string', description: '开户银行' },
        period_start: { type: 'string', format: 'date', description: '流水起始日期' },
        period_end: { type: 'string', format: 'date', description: '流水截止日期' },
        monthly_entries: {
          type: 'array',
          items: {
            type: 'object',
            properties: {
              date: { type: 'string', format: 'date', description: '交易日期' },
        ...
        average_monthly_income: { type: 'number', description: '月均工资收入' }
      },
      required: ['account_holder', 'bank_name', 'average_monthly_income']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请从工资银行流水中提取账户持有人、开户银行、流水期间和每月工资入账记录。校验要点：1）工资入账记录是否来自与误工证明、劳动合同中一致的用人单位；2）月均工资是否与收入证明中声称的月薪大致一致；3）事故后是否出现工资减少或停发情况（佐证误工损失）。'
  },
  {
    id: 'mat-33',
    name: '纳税证明',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        id_number: { type: 'string', description: '身份证号' },
        tax_period_start: { type: 'string', format: 'date', description: '纳税期间起' },
        tax_period_end: { type: 'string', format: 'date', description: '纳税期间止' },
    ...
    required: false,
    aiAuditPrompt: '请从纳税证明中提取纳税人姓名、身份证号、纳税期间、收入总额、已缴税额和出具机关。校验要点：1）纳税记录中的收入水平是否与收入证明、银行流水一致；2）纳税期间是否覆盖事故前的连续收入时段；3）通过税额反推的税前收入是否与声称的月薪匹配。'
  },
  {
    id: 'mat-34',
    name: '护理人员收入证明',
    description: '护理人员（非专业护工）所在单位出具的收入证明，用于计算护理费中的误工损失。',
    jsonSchema: JSON.stringify({
      type: 'object',
      properties: {
        employer_name: { type: 'string', description: '用人单位名称' },
    ...
    required: false,
    aiAuditPrompt: '请从护理人员收入证明中提取单位名称、护理人员姓名、职务和月收入。校验要点：1）护理人员是否为被保险人的亲属或朋友（非专业护工）；2）月收入水平是否合理；3）护理天数对应的误工损失=月收入÷21.75×护理天数。'
  },
  ...
  // ══════════════════════════════════════════════════
  {
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        appraisal_date: { type: 'string', format: 'date', description: '鉴定日期' },
        subject_name: { type: 'string', description: '被鉴定人姓名' },
        disability_grade: { type: 'string', description: '伤残等级（一级~十级）' },
      ...
      required: ['institution_name', 'subject_name', 'disability_grade', 'conclusions']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请从伤残鉴定意见书中提取鉴定机构、文书编号、鉴定日期、被鉴定人、伤残等级、伤情描述和鉴定结论。校验要点：1）鉴定机构是否具有司法鉴定资质（应在司法行政部门登记）；2）伤残等级与伤情描述是否匹配（如十级为最轻，一级为最重）；3）鉴定日期是否在伤情稳定后（一般出院后3-6个月）；4）伤残等级直接影响残疾赔偿金计算（十级=10%，九级=20%，依此类推）。'
  },
  {
    id: 'mat-36',
    name: '劳动能力鉴定结论',
    description: '劳动能力鉴定委员会出具的劳动能力鉴定结论通知书，用于工伤案件。',
    jsonSchema: JSON.stringify({
      type: 'object',
      properties: {
        ...
        nursing_dependency_level: { type: 'string', description: '生活自理障碍等级' },
        conclusion_date: { type: 'string', format: 'date', description: '鉴定日期' }
      },
      required: ['committee_name', 'subject_name', 'disability_level']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请从劳动能力鉴定结论中提取鉴定委员会、被鉴定人、用人单位、伤残等级、生活自理障碍等级和鉴定日期。校验要点：1）鉴定结论是否由设区的市级以上劳动能力鉴定委员会出具；2）与认定工伤决定书中的人员信息是否一致；3）伤残等级是否合理。'
  },
  {
    id: 'mat-37',
    name: '残疾辅助器具证明',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        device_type: { type: 'string', description: '器具类型（假肢/矫形器/轮椅/助听器等）' },
        ...
        issue_date: { type: 'string', format: 'date', description: '出具日期' }
      },
      required: ['institution_name', 'subject_name', 'device_type']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请从残疾辅助器具证明中提取配制机构、申请人、器具类型、规格、更换周期和价格限额。校验要点：1）器具类型是否与伤残情况匹配；2）配制机构是否具有相应资质；3）价格限额是否在国产普及型标准范围内；4）更换周期对于计算后续费用很重要。'
  },
  {
    id: 'mat-38',
    name: '护理依赖鉴定',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        institution_name: { type: 'string', description: '鉴定机构名称' },
        subject_name: { type: 'string', description: '被鉴定人姓名' },
        nursing_dependency_level: { type: 'string', description: '护理依赖等级（完全/大部分/部分护理依赖）' },
        evaluation_date: { type: 'string', format: 'date', description: '评定日期' },
        daily_activities_assessment: { type: 'string', description: '日常生活能力评估' },
        conclusions: { type: 'string', description: '鉴定结论' }
      },
      required: ['institution_name', 'subject_name', 'nursing_dependency_level']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请从护理依赖鉴定中提取鉴定机构、被鉴定人、护理依赖等级、评定日期和日常生活能力评估结论。校验要点：1）护理依赖等级分为完全（100%）、大部分（80%）和部分（50%）三级；2）鉴定机构是否有资质；3）等级认定与后续护理费计算标准直接相关。'
  },
  ...
  // ══════════════════════════════════════════════════
  {
    id: 'mat-39',
    name: '居住证明/暂住证',
    description: '证明被害人在城镇地区连续居住满一年以上的居住证明或暂住证，用于适用城镇标准计算赔偿。',
    jsonSchema: JSON.stringify({
      type: 'object',
      properties: {
        name: { type: 'string', description: '姓名' },
        ...
        valid_from: { type: 'string', format: 'date', description: '有效期起始' },
        valid_until: { type: 'string', format: 'date', description: '有效期截止' },
        issuing_authority: { type: 'string', description: '发证机关' }
      },
      required: ['name', 'residence_address']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请提取居住证明中的姓名、身份证号、居住地址、有效期限和发证机关。校验要点：1）是否在城镇连续居住满一年以上（可结合居住证有效期判断）；2）如果户籍为农村但在城镇居住满一年且有固定收入，可适用城镇标准计算赔偿金；3）居住证是否在有效期内。'
  },
  {
    id: 'mat-40',
    name: '被扶养人身份证明',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        birth_date: { type: 'string', format: 'date', description: '出生日期' },
        id_number: { type: 'string', description: '身份证号' },
        relationship_to_victim: { type: 'string', description: '与受害人关系' },
        dependent_type: { type: 'string', description: '被扶养人类型（未成年子女/无劳动能力配偶/年迈父母等）' }
      },
      required: ['dependent_name', 'birth_date', 'relationship_to_victim']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请提取被扶养人身份证明中的姓名、性别、出生日期、身份证号、与受害人关系和被扶养人类型。校验要点：1）未成年人扶养至18周岁（扶养年限=18-现年龄）；2）无劳动能力又无其他生活来源的成年人扶养20年，60周岁以上每增加一岁减少一年，75周岁以上为5年；3）多个被扶养人的年赔偿总额不超过上一年度城镇/农村居民人均消费支出。'
  },
  {
    id: 'mat-41',
    name: '扶养关系证明',
    ...
    jsonSchema: JSON.stringify({
      type: 'object',
      properties: {
        supporter_name: { type: 'string', description: '扶养人（受害人）姓名' },
        dependent_name: { type: 'string', description: '被扶养人姓名' },
        relationship: { type: 'string', description: '扶养关系' },
        other_supporters_count: { type: 'number', description: '其他扶养义务人数' },
        issuing_authority: { type: 'string', description: '出具机关' },
        issue_date: { type: 'string', format: 'date', description: '开具日期' }
      },
    ...
    required: false,
    aiAuditPrompt: '请提取扶养关系证明中的扶养人、被扶养人、扶养关系、其他扶养义务人数和出具机关。校验要点：1）确认扶养关系真实性（如父母子女关系、配偶关系）；2）其他共同扶养义务人数直接影响赔偿金分摊（被扶养人生活费÷共同扶养人数）；3）出具机关是否为户籍所在地村委会/居委会或民政部门。'
  },
  {
    id: 'mat-42',
    name: '被扶养人无收入证明',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        dependent_name: { type: 'string', description: '被扶养人姓名' },
        id_number: { type: 'string', description: '身份证号' },
        has_income: { type: 'boolean', description: '是否有收入（应为false）' },
        reason_no_income: { type: 'string', description: '无收入原因（残疾/年幼/年迈等）' },
        issuing_authority: { type: 'string', description: '出具机关' },
        issue_date: { type: 'string', format: 'date', description: '开具日期' }
      },
    ...
    required: false,
    aiAuditPrompt: '请提取被扶养人无收入证明中的被扶养人姓名、身份证号、无收入原因和出具机关。校验要点：1）无收入原因是否合理（未成年、在校学生、残疾人、年迈丧失劳动能力等）；2）确认被扶养人确实丧失劳动能力或无其他生活来源；3）证明是否由相关部门或社区出具并盖章。'
  },
  ...
  // ══════════════════════════════════════════════════
  {
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        id_number: { type: 'string', description: '身份证号' },
        death_date: { type: 'string', format: 'date', description: '死亡日期' },
    ...
    required: false,
    aiAuditPrompt: '请从死亡证明中提取死者姓名、身份证号、死亡日期、死亡原因、死亡地点和出具机构。校验要点：1）死亡原因是否与事故存在因果关系（如交通事故致死、工伤致死）；2）死亡日期与事故日期的逻辑关系是否合理；3）出具机构应为医院或公安机关。'
  },
  {
    id: 'mat-44',
    name: '户籍注销证明',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        deceased_name: { type: 'string', description: '死者姓名' },
        id_number: { type: 'string', description: '身份证号' },
        cancellation_date: { type: 'string', format: 'date', description: '户籍注销日期' },
        issuing_authority: { type: 'string', description: '出具机关（派出所）' }
      },
      required: ['deceased_name', 'cancellation_date']
    }, null, 2),
    required: false,
    aiAuditPrompt: '请从户籍注销证明中提取死者姓名、身份证号、户籍注销日期和出具机关。校验要点：1）户籍注销日期应在死亡日期之后；2）姓名和身份证号与死亡证明一致；3）出具机关应为户籍所在地公安派出所。'
  },
  {
    id: 'mat-45',
    name: '亲属关系证明',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        relationship: { type: 'string', description: '亲属关系（配偶/子女/父母等）' },
        issuing_authority: { type: 'string', description: '出具机关' },
        issue_date: { type: 'string', format: 'date', description: '开具日期' }
      },
    ...
    required: false,
    aiAuditPrompt: '请从亲属关系证明中提取死者姓名、申请人姓名、亲属关系、出具机关和开具日期。校验要点：1）确认申请人为法定赔偿权利人（配偶、父母、子女为第一顺序继承人）；2）亲属关系真实性由户籍所在地相关部门证明；3）若有多个权利人，需全部出具关系证明。'
  },
  ...
  // ══════════════════════════════════════════════════
  {
    id: 'mat-46',
    name: '调解协议书/判决书',
    description: '交通事故调解协议、人民法院民事判决书或调解书，明确赔偿金额和各项分项。',
    jsonSchema: JSON.stringify({
      type: 'object',
      properties: {
        case_number: { type: 'string', description: '案号/文书编号' },
        document_type: { type: 'string', description: '文书类型（调解协议/民事判决书/民事调解书）' },
        parties: {
          type: 'array',
          items: {
            type: 'object',
            properties: {
              role: { type: 'string', description: '当事人角色（原告/被告/申请人/被申请人）' },
              name: { type: 'string', description: '姓名/名称' }
            }
          },
          description: '当事人信息'
        },
        agreement_date: { type: 'string', format: 'date', description: '协议/判决日期' },
        compensation_items: {
          type: 'array',
          items: {
            type: 'object',
    ...
    required: false,
    aiAuditPrompt: '请从调解协议书或判决书中提取案号、文书类型、各方当事人、协议/判决日期、赔偿明细（各项赔偿项目及金额）和赔偿总额。校验要点：1）各项赔偿金额之和是否等于赔偿总额；2）赔偿项目是否完整（医疗费、误工费、护理费、残疾赔偿金等）；3）被保险人在案件中的角色及应承担的赔偿比例。'
  },
  {
    id: 'mat-47',
    name: '律师费发票及合同',
    ...
    jsonSchema: JSON.stringify({
      ...
      properties: {
        ...
        contract_date: { type: 'string', format: 'date', description: '合同签订日期' },
        fee_type: { type: 'string', description: '收费方式（固定/风险代理/计时等）' },
    ...
    required: false,
    aiAuditPrompt: '请从律师费发票及合同中提取律师事务所名称、律师姓名、合同日期、收费方式、律师费金额和发票信息。校验要点：1）律师费是否在当地律师收费指导价标准范围内；2）律师服务合同与发票金额是否一致；3）发票是否为正规律师事务所开具的增值税发票。'
  }
];
...
export const MOCK_CLAIM_ITEMS: ClaimItem[] = [
  {
    ...
    materialIds: ['mat-1', 'mat-2', 'mat-6', 'mat-12', 'mat-13', 'mat-20', 'mat-21']
  },
  {
    id: 'item-2',
    name: '意外伤残理赔',
    description: '针对因意外导致的伤残进行理赔。',
    materialIds: ['mat-1', 'mat-2', 'mat-6', 'mat-8', 'mat-13', 'mat-20', 'mat-21', 'mat-35']
  }
];
